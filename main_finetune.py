...
if __name__ == '__main__':
    ...
    for iter in range(args.epochs):
        t_start = time.time()
        w, loss,lr,step = train_1(step,args=args, net=copy.deepcopy(net).to(args.device), dataset=dataset_train,  learning_rate=lr)
        t_end = time.time()   
    ...
    for iter in range(epoch_finetune):
        ...
        acc_test.append(acc_t.item())
        loss_test.append(loss_t)
    print(" Finetuned {:3d},Testing accuracy: {:.2f},Time Elapsed:  {:.2f}s \n".format(iter, acc_t, t_end - t_start))


    # save model
    rootpath1 = './log/Finetune/Model/'
    ...
    accfile = open(rootpath3 + 'Finetune_accfile_model_{}_data_{}_remove_{}_epoch_{}_seed{}.dat'.
    ...
    plt.savefig(rootpath3 + 'Finetune_plot_model_{}_data_{}_remove_{}_epoch_{}_seed{}.png'.format(
        args.model,args.dataset, args.num_forget,args.epochs,args.seed))
    # save Loss
    rootpath4 = './log/Finetune/losstest/'
    if not os.path.exists(rootpath4):
        os.makedirs(rootpath4)
    lossfile = open(rootpath4 + 'Finetune_lossfile_model_{}_data_{}_remove_{}_epoch_{}_seed{}.dat'.
                   format(args.model,args.dataset, args.num_forget,args.epochs,args.seed), "w")
    ...
    plt.savefig(rootpath4 + 'Finetune_plot_model_{}_data_{}_remove_{}_epoch_{}_seed{}.png'.format(
        args.model,args.dataset, args.num_forget,args.epochs,args.seed))
    ...
    lossfile.close()
    # acc 
    rootpath = './log/Finetune/accforget/'
    if not os.path.exists(rootpath):
        os.makedirs(rootpath)  
    accfile = open(rootpath + 'Finetune_accfile_model_{}_data_{}_remove_{}_epoch_{}_seed{}.dat'.format(
    args.model, args.dataset, args.num_forget, args.epochs, args.seed), 'w')
    ...
    lossfile.close()
    # acc
    rootpath = './log/Finetune/accremain/'
    if not os.path.exists(rootpath):
        os.makedirs(rootpath)  
    accfile = open(rootpath + 'Finetune_accfile_model_{}_data_{}_remove_{}_epoch_{}_seed{}.dat'.format(
    args.model, args.dataset, args.num_forget, args.epochs, args.seed), 'w')
