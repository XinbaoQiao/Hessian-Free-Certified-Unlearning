...
from models.Update_neggrad import  train,train_neg
...
if __name__ == '__main__':
    ...
    for iter in range(args.epochs):
        t_start = time.time()
        w, loss,lr,step = train(step,args=args, net=copy.deepcopy(net).to(args.device), dataset=dataset_train,  learning_rate=lr)
        t_end = time.time()   
    ...
    for iter in range(epoch_neg):
        ...
        acc_test.append(acc_t.item())
        loss_test.append(loss_t)
    print(" NegGrad {:3d},Testing accuracy: {:.2f},Time Elapsed:  {:.2f}s \n".format(iter, acc_t, t_end - t_start))


    # save model
    rootpath1 = './log/NegGrad/Model/'
    if not os.path.exists(rootpath1):
        os.makedirs(rootpath1)
    torch.save(net.state_dict(),  rootpath1+ 'NegGrad_model_{}_data_{}_remove_{}_epoch_{}_seed{}.pth'.format(args.model,args.dataset, args.num_forget,args.epochs,args.seed))

    # save acc
    rootpath3 = './log/NegGrad/ACC/'
    if not os.path.exists(rootpath3):
        os.makedirs(rootpath3)
    accfile = open(rootpath3 + 'NegGrad_accfile_model_{}_data_{}_remove_{}_epoch_{}_seed{}.dat'.
    ...
    plt.savefig(rootpath3 + 'NegGrad_plot_model_{}_data_{}_remove_{}_epoch_{}_seed{}.png'.format(
        args.model,args.dataset, args.num_forget,args.epochs,args.seed))
    # save Loss
    rootpath4 = './log/NegGrad/losstest/'
    if not os.path.exists(rootpath4):
        os.makedirs(rootpath4)
    lossfile = open(rootpath4 + 'NegGrad_lossfile_model_{}_data_{}_remove_{}_epoch_{}_seed{}.dat'.
                   format(args.model,args.dataset, args.num_forget,args.epochs,args.seed), "w")
    ...
    plt.savefig(rootpath4 + 'NegGrad_plot_model_{}_data_{}_remove_{}_epoch_{}_seed{}.png'.format(
        args.model,args.dataset, args.num_forget,args.epochs,args.seed))
    ...
    rootpath = './log/NegGrad/lossforget/'
    if not os.path.exists(rootpath):
        os.makedirs(rootpath)  
    lossfile = open(rootpath + 'NegGrad_lossfile_model_{}_data_{}_remove_{}_epoch_{}_seed{}.dat'.format(
    args.model, args.dataset, args.num_forget, args.epochs, args.seed), 'w')
    lossfile.write(str(forget_loss_list))
    lossfile.close()
    # acc 
    rootpath = './log/NegGrad/accforget/'
    if not os.path.exists(rootpath):
        os.makedirs(rootpath)  
    accfile = open(rootpath + 'NegGrad_accfile_model_{}_data_{}_remove_{}_epoch_{}_seed{}.dat'.format(
    args.model, args.dataset, args.num_forget, args.epochs, args.seed), 'w')
    ...
    rootpath = './log/NegGrad/lossremain/'
    if not os.path.exists(rootpath):
        os.makedirs(rootpath)  
    lossfile = open(rootpath + 'NegGrad_lossfile_model_{}_data_{}_remove_{}_epoch_{}_seed{}.dat'.format(
    args.model, args.dataset, args.num_forget, args.epochs, args.seed), 'w')
    ...
    lossfile.close()
    # acc
    rootpath = './log/NegGrad/accremain/'
    if not os.path.exists(rootpath):
        os.makedirs(rootpath)  
    accfile = open(rootpath + 'NegGrad_accfile_model_{}_data_{}_remove_{}_epoch_{}_seed{}.dat'.format(
    args.model, args.dataset, args.num_forget, args.epochs, args.seed), 'w')
